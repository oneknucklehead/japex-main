...
export default function CarCardFirst({ car }: Props) {
  ...
  return (
    <Link
      href={href}
      className={`card block h-full transition-all duration-200 ${
    ...
    >
      <motion.div
        ...
        className="relative group cursor-pointer flex flex-col rounded-[20px] sm:rounded-3xl md:rounded-[28px] overflow-hidden w-full h-full"
        initial="rest"
        whileHover="hover"
        animate="rest"
        style={{
      ...
      >
        ...
        <motion.div
          className="pointer-events-none absolute inset-0 rounded-[20px] sm:rounded-3xl md:rounded-[28px] z-20"
          variants={{
            rest: { opacity: 0 },
            hover: { opacity: 1 },
          }}
        ...
        <div className="relative flex items-center justify-center">
          <div className="relative w-full aspect-video overflow-hidden">
            ...
            <div className="absolute font-dm-sans top-3 left-3 sm:top-4 sm:left-4 flex gap-2">
              {car?.availability === "Sold out" ? (
                <span className="bg-gray-800 text-white text-[10px] sm:text-xs font-semibold px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-full">
                  Sold
                </span>
              ) : car?.availability === "Coming soon" ? (
                <span className="bg-amber-500 text-white text-[10px] sm:text-xs font-semibold px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-full">
                  Coming Soon
                </span>
              ) : !!car?.was_price ? (
                <span className="bg-brand-primary text-white text-[10px] sm:text-xs font-semibold px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-full">
                  Save {formatPrice(car.was_price - car.price)}
                </span>
              ) : car?.is_featured ? (
                <span className="bg-brand-primary text-white text-[10px] sm:text-xs font-semibold px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-full">
                  Featured
        ...
        <div className="flex-1 flex flex-col">
          ...
          <motion.div className="relative w-full h-full">
            <div className="w-full font-bricolage h-full flex flex-col gap-1 items-center justify-start text-white p-3 sm:p-4">
              <div className="flex items-center justify-center flex-col w-full min-w-0">
                {!!car?.was_price && (
                  <BlurRevealText delay={0.2}>
                    <div className="font-dm-sans">
                      <span className="bg-brand-primary text-white text-xs sm:text-sm font-semibold px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-full">
                        On sale!
                      </span>
                    </div>
                  </BlurRevealText>
                ...
                <BlurRevealText delay={0.3}>
                  <h3 className="text-center font-semibold text-[#CA281C] text-base sm:text-lg md:text-xl uppercase">
                    {car?.make}
                  </h3>
                </BlurRevealText>
                <BlurRevealText delay={0.4}>
                  <h1 className="text-center font-extrabold text-xl sm:text-2xl md:text-3xl leading-tight wrap-break-word">
                    {car?.year} {car?.model}
                  </h1>
                </BlurRevealText>
                <div className="flex items-center gap-2 min-h-6 sm:min-h-7">
                  <BlurRevealText delay={0.5}>
                    <h3 className="text-center text-xs sm:text-sm md:text-base">
                      {car?.variant}
                    </h3>
                  </BlurRevealText>
                </div>
                ...
                <div className="flex flex-wrap items-center justify-center gap-x-2 gap-y-1 my-2 text-xs sm:text-sm">
                  <BlurRevealText delay={0.6}>
                    <p className="text-[#A2A2A2] whitespace-nowrap">
                      {formatOdometer(car?.odometer_km)}
                    </p>
                  </BlurRevealText>
                  <BlurRevealText delay={0.65}>
                    <div className="w-1 h-1 bg-[#CA281C] rounded-full" />
                  </BlurRevealText>
              ...
              <div className="w-full mt-auto flex justify-start items-start flex-col">
                <motion.div
                  whileTap={{ scale: 0.98 }}
                  className="mt-auto cursor-pointer flex items-center gap-2 bg-brand-primary hover:bg-[#CA281C]/80 transition-all duration-300 p-1.5 sm:p-2 rounded-full justify-between w-full min-w-0"
                >
                    ...
                    <div className="flex flex-col items-start min-w-0">
                      {car?.availability === "Sold out" ? (
                        ...
                        <div className="flex flex-col items-start ml-1.5 sm:ml-2">
                          {!!car?.was_price && (
                            <span className="text-[10px] sm:text-xs text-white/70 line-through leading-none">
                              {formatPrice(car.was_price)}
                            </span>
                          )}
                          <span className="text-lg sm:text-xl md:text-2xl font-bold leading-tight whitespace-nowrap">
                    ...
                    <motion.div className="flex text-xs sm:text-sm group/button items-center md:gap-1 rounded-full bg-white p-1 font-semibold text-black shrink-0">
                      ...
                      <span>
                        <svg
                          ...
                          className="w-5 h-5 sm:w-6 sm:h-6 group-hover/button:rotate-45 transition-all duration-300"
                        >
                          <path d="M7 7h10v10" />
                          <path d="M7 17 17 7" />
                        </svg>
